...
class StopWatch extends Component {
  ...
  startStopButton() {
    let style = this.state.running
      ? styles.stopButton
      : styles.startButton;
    return (
      <TouchableHighlight underlayColor="gray" onPress={this.handleStartPress.bind(this)} style={[styles.button, style]}>
        <Text>
          {this.state.running
            ? 'Stop'
            : 'Start'}
  ...
  handleLapPress() {
    if (!this.state.running) {
      this.setState({timeElapsed: null, startTime: null, laps: []});
      return;
    }
...
const styles = StyleSheet.create({
  container: {
    flex: 1, // Fil the entire the screen
    alignItems: 'stretch'
  },
  header: { // yellow
    flex: 1
